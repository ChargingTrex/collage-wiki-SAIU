...
//
// Swizzled (ejected, not wrapped) from @docusaurus/theme-classic — same
// reasoning as the DocSidebarItem/Link swizzle: rendering an icon before the
// label requires changing what's inside the sidebar item, which --wrap can't
// do. Kept a thin diff from upstream on purpose: same structure, same class
// names, one addition — reading `item.customProps.icon` (the same
// `sidebar_custom_props.icon` frontmatter convention as Link, see
// src/data/sidebarIcons.js) and rendering the matching lucide icon before the
// category label. Needed because a club that becomes a folder (e.g. once it
// gets an Events subpage) turns from a `type: 'doc'` sidebar item into a
// `type: 'category'` one — without this, its icon would silently disappear
// since upstream's Category component never reads `customProps` at all.
// Categories with no icon set render exactly as upstream.
...
import {SIDEBAR_ICONS} from '@site/src/data/sidebarIcons';
...
function CategoryLinkLabel({label, icon}) {
  ...
  return (
    <span className={styles.categoryLinkLabel}>
      {Icon && <Icon className={styles.categoryIcon} aria-hidden="true" />}
      <span className={styles.categoryLinkText}>{label}</span>
    </span>
  );
}
    ...
    <li
      ...
      <div
        ...
        <Link
          ...
          {...props}>
          <CategoryLinkLabel label={label} icon={customProps?.icon} />
        </Link>
        {href && collapsible && (
          <CollapseButton
            collapsed={collapsed}
